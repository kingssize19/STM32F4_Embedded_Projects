...
/* Definitions for read_hcsr04 */
osThreadId_t read_hcsr04Handle;
const osThreadAttr_t read_hcsr04_attributes = {
  .name = "read_hcsr04",
  .stack_size = 128 * 4,
...
void Read_HCSR04(void *argument);
...
int main(void)
{
  ...
  /* Create the thread(s) */
  /* creation of read_hcsr04 */
  read_hcsr04Handle = osThreadNew(Read_HCSR04, NULL, &read_hcsr04_attributes);
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOA, GREEN_LED_Pin|GPIO_PIN_6|RED_LED_Pin|GPIO_PIN_9, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(BUZZER_GPIO_Port, BUZZER_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : GREEN_LED_Pin PA6 RED_LED_Pin PA9 */
  GPIO_InitStruct.Pin = GREEN_LED_Pin|GPIO_PIN_6|RED_LED_Pin|GPIO_PIN_9;
...
/* USER CODE BEGIN Header_Read_HCSR04 */
/**
  * @brief  Function implementing the read_hcsr04 thread.
  * @param  argument: Not used
  * @retval None
  */
/* USER CODE END Header_Read_HCSR04 */
void Read_HCSR04(void *argument)
{
  /* USER CODE BEGIN 5 */
  ...
  for(;;)
  {
	  HAL_GPIO_WritePin(TRIG_PORT, TRIG_PIN, GPIO_PIN_RESET);
	  		...
	  		sprintf(uartBuf, "Distance (cm)  = %lu\r\n", distance);
      ...
      osDelay(100); // Ölçümler arası bekleme
  }
  /* USER CODE END 5 */
}
...
void StartBuzzer(void *argument)
{
  ...
  /* Infinite loop */
	for (;;) {
	        ...
	        { // Eğer mesafe 20 cm'den azsa
	            HAL_GPIO_WritePin(BUZZER_GPIO_Port, BUZZER_PIN, GPIO_PIN_SET); // Buzzer'ı aç
	            osDelay(100);
	            HAL_GPIO_WritePin(BUZZER_GPIO_Port, BUZZER_PIN, GPIO_PIN_RESET);
	            osDelay(1000);
	        }
	        ...
	        osDelay(100); // Kontrol arası bekleme
	    }
...
void StartLedBlink(void *argument)
{
  ...
  {
      if (distance < 10) {
          HAL_GPIO_WritePin(GPIOA, RED_LED, GPIO_PIN_SET); // Çok yakın mesafe
          HAL_GPIO_WritePin(GPIOA, YELLOW_LED, GPIO_PIN_SET);
          HAL_GPIO_WritePin(GPIOA, GREEN_LED, GPIO_PIN_SET);
      } else if (distance >= 10 && distance < 20) {
          HAL_GPIO_WritePin(GPIOA, RED_LED, GPIO_PIN_RESET);
          HAL_GPIO_WritePin(GPIOA, YELLOW_LED, GPIO_PIN_SET); // Orta mesafe
          HAL_GPIO_WritePin(GPIOA, GREEN_LED, GPIO_PIN_SET);
      } else if (distance >= 20 && distance < 30) {
          HAL_GPIO_WritePin(GPIOA, RED_LED, GPIO_PIN_RESET);
          HAL_GPIO_WritePin(GPIOA, YELLOW_LED, GPIO_PIN_RESET);
          HAL_GPIO_WritePin(GPIOA, GREEN_LED, GPIO_PIN_SET); // Uzak mesafe
      } else {
          HAL_GPIO_WritePin(GPIOA, RED_LED, GPIO_PIN_RESET);
          HAL_GPIO_WritePin(GPIOA, YELLOW_LED, GPIO_PIN_RESET);
          HAL_GPIO_WritePin(GPIOA, GREEN_LED, GPIO_PIN_RESET); // Güvenli mesafe
      }

      osDelay(100); // LED kontrol frekansı
  }
